...
CRON_ENTRY="*/5 * * * * /root/rp_status/check-github-changes.sh >> /tmp/github-check.log 2>&1"

# Check if the cron job already exists
if crontab -l 2>/dev/null | grep -q "check-github-changes.sh"; then
    echo "Cron job already exists. Updating..."
    # Remove existing cron job and add new one
    (crontab -l 2>/dev/null | grep -v "check-github-changes.sh"; echo "$CRON_ENTRY") | crontab -
else
